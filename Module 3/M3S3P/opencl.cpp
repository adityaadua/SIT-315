...
int main(int argc, char **argv)
{
    ...
    init(v_out, SZ); // Initialize output vector

    size_t global[1] = {(size_t)SZ}; // Global size for OpenCL kernel execution

    print(v1, SZ); // Print input vector 1
    print(v2, SZ); // Print input vector 2
    ...
    print(v_out, SZ);                                                      // Print output vector
...
cl_program build_program(cl_context ctx, cl_device_id dev, const char *filename)
{
    ...
    if (err < 0)
    {
        // print program build log If build fails
